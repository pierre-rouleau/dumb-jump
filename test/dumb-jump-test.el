...
;; ---------------------------------------------------------------------------
;; Tests for new functionality added in PR #5 (detect-prompt-in-test)
;; --------------------------------------------------------------------

;; Tests for dumb-jump-use-space-bracket-exp-for
(ert-deftest dumb-jump-use-space-bracket-exp-for-grep-test ()
  "Test that dumb-jump-use-space-bracket-exp-for returns t for grep when enabled."
...
(ert-deftest dumb-jump-use-space-bracket-exp-for-non-grep-test ()
  "Test that dumb-jump-use-space-bracket-exp-for returns nil for ag/rg."
  (let ((dumb-jump-force-using-space-bracket-exp t))
    (should-not (dumb-jump-use-space-bracket-exp-for 'ag))
    (should-not (dumb-jump-use-space-bracket-exp-for 'rg))))

(ert-deftest dumb-jump-use-space-bracket-exp-for-disabled-test ()
  "Test that dumb-jump-use-space-bracket-exp-for returns nil when disabled."
...
;; Tests for space bracket expansion in populate-regex
(ert-deftest dumb-jump-populate-regex-space-bracket-gnu-grep-test ()
  "Test that \\s is replaced with [[:space:]] for gnu-grep when enabled."
  (let ((dumb-jump-force-using-space-bracket-exp t))
    (should (string= (dumb-jump-populate-regex "\\(defun\\s+JJJ\\j" "test" 'gnu-grep)
                     "\\(defun[[:space:]]+test($|[^a-zA-Z0-9\\?\\*-])"))))

(ert-deftest dumb-jump-populate-regex-space-bracket-grep-test ()
  "Test that \\s is replaced with [[:space:]] for grep when enabled."
  (let ((dumb-jump-force-using-space-bracket-exp t))
    (should (string= (dumb-jump-populate-regex "\\(defun\\s+JJJ\\j" "test" 'grep)
                     "\\(defun[[:space:]]+test($|[^a-zA-Z0-9\\?\\*-])"))))

(ert-deftest dumb-jump-populate-regex-space-bracket-disabled-test ()
  "Test that \\s is NOT replaced when disabled."
  (let ((dumb-jump-force-using-space-bracket-exp nil))
    (should (string= (dumb-jump-populate-regex "\\(defun\\s+JJJ\\j" "test" 'gnu-grep)
                     "\\(defun\\s+test($|[^a-zA-Z0-9\\?\\*-])"))))

(ert-deftest dumb-jump-populate-regex-space-bracket-ag-test ()
  "Test that \\s is NOT replaced for ag (even when enabled)."
  (let ((dumb-jump-force-using-space-bracket-exp t))
    (should (string= (dumb-jump-populate-regex "\\(defun\\s+JJJ\\j" "test" 'ag)
                     "\\(defun\\s+test(?![a-zA-Z0-9\\?\\*-])"))))

;; Tests for new elisp regex patterns
(ert-deftest dumb-jump-find-rules-elisp-cl-defgeneric-test ()
  "Test that cl-defgeneric is recognized in elisp find rules."
  (let* ((rules (--filter (and (string= (plist-get it :language) "elisp")
                               (string= (plist-get it :type) "function"))
                          dumb-jump-find-rules))
         (first-rule (car rules))
         (regex (plist-get first-rule :regex)))
    ;; Check that the regex includes cl-defgeneric
    (should (s-contains? "cl-defgeneric" regex))))

(ert-deftest dumb-jump-find-rules-elisp-cl-defmethod-test ()
  "Test that cl-defmethod is recognized in elisp find rules."
  (let* ((rules (--filter (and (string= (plist-get it :language) "elisp")
                               (string= (plist-get it :type) "function"))
                          dumb-jump-find-rules))
         (first-rule (car rules))
         (regex (plist-get first-rule :regex)))
    ;; Check that the regex includes cl-defmethod
    (should (s-contains? "cl-defmethod" regex))))

(ert-deftest dumb-jump-find-rules-elisp-defhydra-test ()
  "Test that defhydra has its own rule in elisp find rules."
  (let* ((hydra-rules (--filter (and (string= (plist-get it :language) "elisp")
                                     (string= (plist-get it :type) "function")
                                     (s-contains? "defhydra" (plist-get it :regex)))
                                dumb-jump-find-rules)))
    ;; There should be at least one rule for defhydra
    (should (> (length hydra-rules) 0))
    ;; Check the regex is correct
    (let ((regex (plist-get (car hydra-rules) :regex)))
      (should (s-contains? "defhydra" regex)))))

(ert-deftest dumb-jump-find-rules-elisp-defvar-local-test ()
  "Test that defvar-local is recognized in elisp variable rules."
  (let* ((rules (--filter (and (string= (plist-get it :language) "elisp")
         ...
         (first-rule (car rules))
         (regex (plist-get first-rule :regex)))
    ;; Check that the regex includes defvar-local
    (should (s-contains? "defvar-local" regex))))

(ert-deftest dumb-jump-find-rules-elisp-defconst-test ()
  "Test that defconst has a dedicated rule in elisp variable rules."
  (let* ((const-rules (--filter (and (string= (plist-get it :language) "elisp")
                                     (string= (plist-get it :type) "variable")
                                     (s-contains? "defconst" (plist-get it :regex)))
                                dumb-jump-find-rules)))
    ;; There should be at least one rule for defconst
    (should (> (length const-rules) 0))))

(ert-deftest dumb-jump-find-rules-elisp-cl-defstruct-test ()
  "Test that cl-defstruct is recognized in elisp type rules."
  (let* ((type-rules (--filter (and (string= (plist-get it :language) "elisp")
                                    (string= (plist-get it :type) "type"))
                               dumb-jump-find-rules)))
    ;; There should be at least one type rule for elisp
    (should (> (length type-rules) 0))
    ;; Check that cl-defstruct is in the regex
    (let ((regex (plist-get (car type-rules) :regex)))
      (should (s-contains? "cl-defstruct" regex)))))

(ert-deftest dumb-jump-find-rules-elisp-cl-deftype-test ()
  "Test that cl-deftype is recognized in elisp type rules."
  (let* ((type-rules (--filter (and (string= (plist-get it :language) "elisp")
                                    (string= (plist-get it :type) "type"))
                               dumb-jump-find-rules)))
    ;; Check that cl-deftype is in the regex
    (let ((regex (plist-get (car type-rules) :regex)))
      (should (s-contains? "cl-deftype" regex)))))

;; Tests for gnu-grep word boundary
(ert-deftest dumb-jump-gnu-grep-word-boundary-exists-test ()
  "Test that dumb-jump-gnu-grep-word-boundary is defined."
  (should (boundp 'dumb-jump-gnu-grep-word-boundary))
  (should (stringp dumb-jump-gnu-grep-word-boundary)))

(ert-deftest dumb-jump-populate-regex-gnu-grep-word-boundary-test ()
  "Test that gnu-grep uses its own word boundary regex."
  (should (string= (dumb-jump-populate-regex "\\j" "test" 'gnu-grep)
                   (concat dumb-jump-gnu-grep-word-boundary))))

;; Regression test: ensure with-mock-forbidding-prompt actually detects prompts
(ert-deftest with-mock-forbidding-prompt-detects-prompt-test ()
...
;; Edge case: test regex with special characters
(ert-deftest dumb-jump-populate-regex-special-chars-test ()
  "Test that special regex characters are properly escaped."
  (should (string= (dumb-jump-populate-regex "JJJ" "$test" 'ag)
                   "\\$test"))
  (should (string= (dumb-jump-populate-regex "JJJ" "test.var" 'ag)
                   "test\\.var")))

;; Boundary test: test with empty and nil inputs
(ert-deftest dumb-jump-use-space-bracket-exp-for-invalid-variant-test ()
  "Test that invalid variant symbols return nil."
  (let ((dumb-jump-force-using-space-bracket-exp t))
    (should-not (dumb-jump-use-space-bracket-exp-for 'invalid-tool))
    (should-not (dumb-jump-use-space-bracket-exp-for 'unknown))))

;; Integration test: verify ag command generation includes "--" separator
(ert-deftest dumb-jump-generate-ag-command-includes-separator-test ()
  "Test that ag commands include the -- separator before regex."
  (let* ((regexes (dumb-jump-get-contextual-regexes "elisp" nil 'ag))
         (command (dumb-jump-generate-ag-command "tester" "blah.el" "." regexes "elisp" nil)))
    ;; Command should contain " -- " separator
    (should (s-contains? " -- " command))))

;; Integration test: verify rg command generation includes "--" separator
(ert-deftest dumb-jump-generate-rg-command-includes-separator-test ()
  "Test that rg commands include the -- separator before regex."
  (let* ((regexes (dumb-jump-get-contextual-regexes "elisp" nil 'rg))
         (command (dumb-jump-generate-rg-command "tester" "blah.el" "." regexes "elisp" nil)))
    ;; Command should contain " -- " separator
    (should (s-contains? " -- " command))))

;; Integration test: verify gnu-grep command generation includes "--" separator
(ert-deftest dumb-jump-generate-gnu-grep-command-includes-separator-test ()
  "Test that gnu-grep commands include the -- separator before regex."
  (let* ((regexes (dumb-jump-get-contextual-regexes "elisp" nil 'gnu-grep))
         (command (dumb-jump-generate-gnu-grep-command "tester" "blah.el" "." regexes "elisp" nil)))
    ;; Command should contain " -- " or just work correctly
    ;; Check the command structure is valid
    (should (stringp command))
    (should (> (length command) 0))))

;; Negative test: verify old patterns that should NOT match
(ert-deftest dumb-jump-find-rules-elisp-three-backslash-removed-test ()
  "Test that the old three-backslash pattern is no longer used."
  (let* ((rules (--filter (and (string= (plist-get it :language) "elisp")
                               (string= (plist-get it :type) "function"))
                          dumb-jump-find-rules))
         (first-rule (car rules))
         (regex (plist-get first-rule :regex)))
    ;; The new regex should start with \( not \\\(
    (should (s-starts-with? "\\(" regex))
    (should-not (s-starts-with? "\\\\\\(" regex))))
