...
VARIANT must be one of: ag, rg, grep, gnu-grep, git-grep, or git-grep-plus-ag."
  (let ((regexes (if (eq type 'elisp-functions)
                     ...
                     (-take 3 dumb-jump--expected-elisp-regexps-templates)
...
;; ---------------------------------------------------------------------------
;; Common Lisp specific tests
;; ---------------------------

(ert-deftest dumb-jump-commonlisp-function-regex-test ()
  "Test that Common Lisp function definition regexes work correctly.
This test validates the fix for removing the unnecessary 3rd backslash
from Common Lisp regexes."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "function")
                                    (s-contains? "defun" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    ;; Verify the regex exists and has correct format
    (should rule)
    (should regex)
    ;; Verify it matches expected Common Lisp function definitions
    (should (string-match-p (dumb-jump-populate-regex regex "my-func" 'grep)
                            "(defun my-func (x)"))
    (should (string-match-p (dumb-jump-populate-regex regex "test-fn" 'grep)
                            "(defmacro test-fn ()"))
    (should (string-match-p (dumb-jump-populate-regex regex "generic-fn" 'grep)
                            "(defgeneric generic-fn (obj)"))
    (should (string-match-p (dumb-jump-populate-regex regex "my-method" 'grep)
                            "(defmethod my-method ((obj class))"))
    (should (string-match-p (dumb-jump-populate-regex regex "accessor" 'grep)
                            "(defsetf accessor (obj)"))))

(ert-deftest dumb-jump-commonlisp-define-macro-regex-test ()
  "Test Common Lisp define-* macro regexes work correctly."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "function")
                                    (s-contains? "define-" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    (should regex)
    ;; Test define-modify-macro
    (should (string-match-p (dumb-jump-populate-regex regex "incf-by" 'grep)
                            "(define-modify-macro incf-by (delta)"))
    ;; Test define-compiler-macro
    (should (string-match-p (dumb-jump-populate-regex regex "optimize-fn" 'grep)
                            "(define-compiler-macro optimize-fn (x)"))
    ;; Test define-setf-expander
    (should (string-match-p (dumb-jump-populate-regex regex "my-place" 'grep)
                            "(define-setf-expander my-place (obj)"))))

(ert-deftest dumb-jump-commonlisp-variable-regex-test ()
  "Test Common Lisp variable definition regexes work correctly."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "variable")
                                    (s-contains? "defvar" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    (should regex)
    ;; Test defvar
    (should (string-match-p (dumb-jump-populate-regex regex "*global*" 'grep)
                            "(defvar *global* 42"))
    ;; Test defparameter
    (should (string-match-p (dumb-jump-populate-regex regex "*param*" 'grep)
                            "(defparameter *param* 100"))
    ;; Test defconstant
    (should (string-match-p (dumb-jump-populate-regex regex "+pi+" 'grep)
                            "(defconstant +pi+ 3.14159"))))

(ert-deftest dumb-jump-commonlisp-symbol-macro-regex-test ()
  "Test Common Lisp define-symbol-macro regex works correctly."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "variable")
                                    (s-contains? "symbol-macro" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    (should regex)
    ;; Test define-symbol-macro
    (should (string-match-p (dumb-jump-populate-regex regex "my-symbol" 'grep)
                            "(define-symbol-macro my-symbol (get-value)"))))

(ert-deftest dumb-jump-commonlisp-type-regex-test ()
  "Test Common Lisp type definition regexes work correctly."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "type")
                                    (s-contains? "defclass" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    (should regex)
    ;; Test defclass
    (should (string-match-p (dumb-jump-populate-regex regex "person" 'grep)
                            "(defclass person ()"))
    ;; Test defstruct
    (should (string-match-p (dumb-jump-populate-regex regex "point" 'grep)
                            "(defstruct point x y"))
    ;; Test deftype
    (should (string-match-p (dumb-jump-populate-regex regex "string-list" 'grep)
                            "(deftype string-list ()"))))

(ert-deftest dumb-jump-commonlisp-condition-regex-test ()
  "Test Common Lisp define-condition regex works correctly."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "type")
                                    (s-contains? "condition" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    (should regex)
    ;; Test define-condition
    (should (string-match-p (dumb-jump-populate-regex regex "my-error" 'grep)
                            "(define-condition my-error (error)"))))

(ert-deftest dumb-jump-commonlisp-word-boundary-test ()
  "Test that Common Lisp regexes properly handle word boundaries.
This ensures symbols with hyphens, asterisks, and other characters
are matched correctly but don't match prefixes."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "function")
                                    (s-contains? "defun" (plist-get it :regex)))
                               dumb-jump-find-rules)))
    ...
    (should (string-match-p (dumb-jump-populate-regex regex "test" 'grep)
                            "(defun test ()"))
    ;; Should match with newline after name
    (should (string-match-p (dumb-jump-populate-regex regex "test" 'grep)
                            "(defun test\n"))
    ;; Should NOT match when the name is a prefix
    (should-not (string-match-p (dumb-jump-populate-regex regex "test" 'grep)
                                "(defun test-something ()"))
    (should-not (string-match-p (dumb-jump-populate-regex regex "test" 'grep)
                                "(defun tester ()"))))

(ert-deftest dumb-jump-commonlisp-case-insensitive-test ()
  "Test that Common Lisp is correctly marked as case-insensitive.
Common Lisp symbols are case-insensitive by default."
  (should (member "commonlisp" dumb-jump--case-insensitive-languages)))

(ert-deftest dumb-jump-commonlisp-populate-regex-all-tools-test ()
  "Test that Common Lisp regexes work correctly with all supported tools.
This ensures the backslash fix works across grep, ag, rg, and git-grep."
  (let ((regex "\\(defun\\s+JJJ\\j")
        (symbol "test-func"))
    ;; Test with grep
    (should (string-match-p (dumb-jump-populate-regex regex symbol 'grep)
                            "(defun test-func (x)"))
    ;; Test with gnu-grep
    (should (string-match-p (dumb-jump-populate-regex regex symbol 'gnu-grep)
                            "(defun test-func (x)"))
    ;; Test with ag
    (should (string-match-p (dumb-jump-populate-regex regex symbol 'ag)
                            "(defun test-func (x)"))
    ;; Test with rg
    (should (string-match-p (dumb-jump-populate-regex regex symbol 'rg)
                            "(defun test-func (x)"))
    ;; Test with git-grep
    (should (string-match-p (dumb-jump-populate-regex regex symbol 'git-grep)
                            "(defun test-func (x)"))))

(ert-deftest dumb-jump-commonlisp-special-chars-in-names-test ()
  "Test Common Lisp regexes with special characters in symbol names.
Common Lisp allows various special characters in symbols."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "function")
                                    (s-contains? "defun" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    ;; Test with asterisks (conventionally used for special/global vars, but valid in function names)
    (should (string-match-p (dumb-jump-populate-regex regex "*special*" 'grep)
                            "(defun *special* ()"))
    ;; Test with plus signs (conventionally used for constants)
    (should (string-match-p (dumb-jump-populate-regex regex "+constant+" 'grep)
                            "(defun +constant+ ()"))))

(ert-deftest dumb-jump-commonlisp-multiline-definition-test ()
  "Test that Common Lisp regexes match definitions split across lines."
  (let* ((rule (car (--filter (and (string= (plist-get it :language) "commonlisp")
                                    (string= (plist-get it :type) "function")
                                    (s-contains? "defun" (plist-get it :regex)))
                               dumb-jump-find-rules)))
         (regex (plist-get rule :regex)))
    (should rule)
    ;; Test with newline after function name
    (should (string-match-p (dumb-jump-populate-regex regex "my-function" 'grep)
                            "(defun my-function\n  (arg1 arg2)"))
    ;; Test with just newline and no args yet
    (should (string-match-p (dumb-jump-populate-regex regex "another-fn" 'grep)
                            "(defun another-fn\n"))))

(ert-deftest dumb-jump-commonlisp-namespace-processing-test ()
  "Test that Common Lisp namespace processing works correctly.
Common Lisp uses colons for package qualifiers."
  ;; Test that package prefix is removed
  (should (string= (dumb-jump-process-symbol-by-lang "commonlisp" "my-package:my-function")
                   "my-function"))
  ;; Test that keyword symbols (starting with :) are not processed
  (should (string= (dumb-jump-process-symbol-by-lang "commonlisp" ":keyword")
                   ":keyword"))
  ;; Test symbols without package qualifier are unchanged
  (should (string= (dumb-jump-process-symbol-by-lang "commonlisp" "simple-function")
                   "simple-function")))

(ert-deftest dumb-jump-commonlisp-regex-backslash-count-test ()
  "Regression test to ensure Common Lisp regexes use exactly 2 backslashes.
This test verifies the fix for removing unnecessary 3rd backslashes.
In Emacs Lisp strings, '\\(' represents an escaped opening paren,
and '\\\(' would represent a backslash followed by an opening paren."
  (let ((rules (--filter (string= (plist-get it :language) "commonlisp")
                         dumb-jump-find-rules)))
    (should (> (length rules) 0))
    ;; Check each commonlisp rule
    (--each rules
      (let ((regex (plist-get it :regex)))
        ;; The regex should contain "\\(" (2 backslashes before paren)
        (should (s-contains? "\\(" regex))
        ;; The regex should NOT contain "\\\(" (3 backslashes before paren)
        (should-not (s-contains? "\\\\\\\(" regex))
        ;; Check for proper word boundary markers
        (should (or (s-contains? "\\b" regex)
                    (s-contains? "\\j" regex)))))))

(ert-deftest dumb-jump-commonlisp-file-extension-test ()
  "Test that Common Lisp file extensions are properly configured."
  ;; Check that .lisp extension maps to commonlisp
  (should (string= (dumb-jump-get-language-by-filename "test.lisp") "commonlisp"))
  ;; Check that .lsp extension maps to commonlisp
  (should (string= (dumb-jump-get-language-by-filename "test.lsp") "commonlisp")))
